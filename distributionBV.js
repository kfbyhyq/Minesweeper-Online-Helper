                ...
                chrome.scripting.executeScript({
                    ...
                    function: function () {
                        try {
                            ...
                            const pa = document.querySelector("#stat_pagination > li.page.active");
                            if (pa) {
                                iniPage = Number(pa.textContent);
                            }
                            BVInterval = setInterval(() => {
                                const pageActive = document.querySelector("#stat_pagination > li.page.active");
                                if (pageActive) { // 有翻页标记，多页
                                    if (pageActive.textContent == pageNum + iniPage) { // 当前页码等于录入页码
                                        const firstInd = document.querySelector("#stat_table_body > tr:nth-child(1) > td:nth-child(1)").textContent;
                                        if (+firstInd == (pageNum + iniPage) * 10 - 9) { // 确认页码正确，防延迟
                                            ...
                                            pageNum++;
                                            const pageLastDisabled = document.querySelector("#stat_pagination > li.last.disabled");
                                            if (pageLastDisabled || pageNum >= maxPage) { // 没有下一页说明到达最后一页
                                                clearInterval(BVInterval);
                                                setTimeout(() => {
                                                    console.log(BVDistribution);
                                                    chrome.runtime.sendMessage({ action: 'sendBVDistribution500', BVDistribution: BVDistribution, level: level });
                                                    saveAsCsv(BVDistribution, 'BVDistribution.csv');
                                                }, t1);
                                            } else { // 否则翻页
                                                setTimeout(() => {
                                    ...
                                    } else if (pageActive.textContent < pageNum + iniPage) { // 页码落后于录入页码就翻页
                                        setTimeout(() => {
                                            const pageNext = document.querySelector("#stat_pagination > li.next");
                                            pageNext.click();
                                        }, t1);
                                    } else if (pageActive.textContent > pageNum + iniPage) { // 页码领先于录入页码就后退一页
                                        setTimeout(() => {
                                            const pagePrev = document.querySelector("#stat_pagination > li.prev");
                                            pagePrev.click();
                                        }, t1);
                                    }
                                } else { // 无页码，只有一页
